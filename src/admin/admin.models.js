...
async function ThemTestCase(MaCH, SQLQuery) {
  try {
    ...
    if (QueryData.statusCode != 200) return QueryData;
    ...
    if (ResultQueryInsert.statusCode != 200) {
      GhiLog(
        `Lỗi Insert TestCase (admin.models) - ${InsertSQLQuery}\t${ResultQueryInsert}`
      );
...
async function SuaTestCase(MaCH, SQLQuery) {
  try {
    ...
    if (ResultQueryInsert.statusCode != 200) {
      GhiLog(
        `Lỗi Update TestCase (admin.models) - ${InsertSQLQuery}\t${ResultQueryInsert}`
      );
      return ResultQueryInsert;
    }
...
async function ThemBaiTap(data) {
  try {
    if (data.TrangThai) data.TrangThai = 1;
