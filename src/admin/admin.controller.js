...
async function ThemTestCase(req, res, next) {
  ...
  else return res.status(400).send({ message: "Thêm testcase thất bại" });
}
...
async function ThemNhom(req, res) {
  try {
    const data = req.body;

    if (!data.MaNhom || !data.TenNhom)
      return res.status(400).send({ message: "Missing fields" });
    const result = await adminModel.ThemNhom(data);
    if (result.statusCode === 200)
      return res.send({
        statusCode: 200,
        message: result.message,
...
async function ThemSinhVien(req, res) {
  try {
    const data = req.body;
    ...
    const result = await adminModel.ThemSinhVien(data);
    if (result.statusCode === 200)
      return res.send({
        statusCode: 200,
        message: result.message,
...
async function DanhSachSinhVien(req, res) {
  let result = await adminModel.DanhSachSinhVien();
  let dsNhom = await adminModel.DanhSachNhom();
  if (result.statusCode === 200) {
...
async function DanhSachNhomSV(req, res) {
